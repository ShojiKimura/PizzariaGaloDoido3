...
async function cadastrarCliente() {
    ...
    try {
        await pool.query("INSERT INTO clientes (nome, telefone, cpf, endereco) VALUES ($1, $2, $3, $4)", [nome, telefone, cpf, endereco]);
...
async function atualizarCliente() {
    ...
    try {
        ...
        await pool.query("UPDATE clientes SET nome = $1, telefone = $2, cpf = $3, endereco = $4 WHERE id = $5", [nome, telefone, cpf, endereco, id]);
...
async function registrarPedido() {
    ...
    if (itens.length === 0)
        return console.log("❌ Nenhum item adicionado. Pedido cancelado.");
    // Promoção automática
    let desconto = 0;
    if (total > 100) {
        desconto = total * 0.1;
    ...
    try {
        const pedidoRes = await pool.query("INSERT INTO pedidos (id_cliente, itens, total, desconto) VALUES ($1, $2, $3, $4) RETURNING id", [idCliente, itens.join("; "), totalFinal, desconto]);
...
async function listarPedidos() {
    ...
    try {
        ...
        `);
        if (res.rows.length === 0)
            return console.log("Nenhum pedido registrado.");
        for (const p of res.rows) {
            console.log(`ID: ${p.id} | Cliente: ${p.cliente} | Itens: ${p.itens} | Total: R$ ${Number(p.total).toFixed(2)} | Status: ${p.status}`);
        }
    }
    catch {
...
async function emitirComprovante(Pedido) {
    const res = (await pool.query(`
    ...
    try {
        await pool.query("INSERT INTO comprovantes (id_pedido, conteudo) VALUES ($1, $2)", [idPedido, texto]);
...
async function historicoCliente() {
    ...
    `, [idCliente]);
    if (res.rows.length === 0)
        return console.log("Nenhum pedido encontrado para este cliente.");
    for (const p of res.rows) {
